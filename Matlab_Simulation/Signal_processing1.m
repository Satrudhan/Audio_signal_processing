...
subplot(4,1,1); title('Original Signal'); hold on; grid on;
subplot(4,1,2); title('Normalized Signal'); hold on; grid on;
subplot(4,1,3); title('Filtered Signal'); hold on; grid on;
subplot(4,1,4); title('Smoothed Signal'); hold on; grid on;
...
while mockSignalIndex + SIGNAL_SIZE - 1 <= totalSamples
    ...
    figure(1);
    subplot(4,1,1); cla; plot(original_signal, 'r'); title('Original Signal');
    xlabel('Samples'); ylabel('Amplitude'); grid on;

    subplot(4,1,2); cla; plot(normalized_signal, 'm'); title('Normalized Signal');
    xlabel('Samples'); ylabel('Amplitude'); grid on;

    subplot(4,1,3); cla; plot(filtered_signal, 'b'); title('Filtered Signal');
    xlabel('Samples'); ylabel('Amplitude'); grid on;

    subplot(4,1,4); cla; plot(smoothed_signal, 'g'); title('Smoothed Signal');
    xlabel('Samples'); ylabel('Amplitude'); grid on;
    ...
    % =================== Spectrogram ===================
    figure(3);
    spectrogram(original_signal, 256, 250, 512, SAMPLE_RATE, 'yaxis');
